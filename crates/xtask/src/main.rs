...
fn render_configuration_doc() -> String {
    ...
    output.push_str("Tmux integration is an add-on. Set `tmux_enabled = true` to start in tmux mode by default. Use `tmux Sessions…` from the command palette or File menu to attach, switch, create, and manage sessions at runtime; tmux-only commands require an active tmux session.\n\n");
...
fn render_default_config_template() -> String {
    ...
    output.push_str("# tmux-only keybind actions require an active tmux session\n");
